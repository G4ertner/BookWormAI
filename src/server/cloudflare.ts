...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      if (!['/api/companion/config','/api/companion/turn','/api/companion/search','/api/companion/exa-key','/api/books/recommendations/config','/api/books/recommendations','/api/audio/config','/api/audio/key','/api/audio/selection','/api/audio/speech'].includes(url.pathname)) return json({error:{code:'NOT_FOUND',message:'Not found.'}},404);
